...
function createHTML5video(videoUrl,ext,cover){
	...
	var html5video = '<video class="cm-video"  x-webkit-airplay="true" webkit-playsinline="true" playsinline="true" x5-video-player-type="h5" autoplay poster="'+cover+'" controls  src="'+videoUrl+'">';
...
function createFlash(flashvar,height){		
	...
	FlashPlayer += '<param name="movie" value="http://www.81.cn/res/34820.files/player.swf">';
	...
	FlashPlayer += 'src="http://www.81.cn/res/34820.files/player.swf" flashvars="'+ flashvar +'" allowfullscreen="true" type="application/x-shockwave-flash">';
	FlashPlayer += '</object>';
	return FlashPlayer;
}
...
function makeCode(){
	...
	var option = {}
	var qrbox = $('#qrcode');
	var qrcode = qrbox.find('div.qrcode-content');
	...
	option = {
		render: 'image',    // render method: 'canvas' or 'image'
		crisp: true,        // render pixel-perfect lines
		minVersion: 1,      // minimum version: 1-40   
		ecLevel: 'Q',       // error correction level: 'L', 'M', 'Q' or 'H'
		size: 500,          // size in pixel
		ratio: null,        // pixel-ratio, null for devicePixelRatio
		fill: '#333',       // code color
		back: '#fff',       // background color
		text: window.location.origin+url,          // content
		rounded: 0,         // roundend corners in pc: 0..100
		quiet: 1,           // quiet zone in modules
		mode: 'image',      // modes: 'plain', 'label' or 'image'
		mSize: 20,          // label/image size and pos in pc: 0..100
		mPosX: 50,
		mPosY: 50,    
		label: '',          // label
		fontname: 'sans',
		fontcolor: '#333',        
		image: icon[0]         // image element
	}
	
	var createCode = function(){
		qrcode.html(kjua(option));
		qrbox.removeClass('pending')
		qrbox.on('click',function(){
			qrbox.toggleClass('active')
		})
	}
	...
	if(icon[0].complete){
		...
		icon.on('load',createCode)
	}
	
   /**/
	var createh5Code = function(){
		h5box.html(kjua(option));
...
;(function(){
    ...
    /*二维码*/
		...
		let kjuapath = document.getElementById('kjuapath').content;
		
		node.setAttribute('src', kjuapath);
...
+function(){
	if(location.host !='www.81.zg.jw'){
		...
		window._bd_share_config={
			"common":{
                // "bdUrl" : 'https://' + window.location.host+ '/' + url,		
